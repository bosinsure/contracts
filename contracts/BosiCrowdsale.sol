 ...
 */
contract BosiCrowdsale is BosiCrowdsaleConfig, Ownable, Pausable, HasNoTokens {
  using SafeMath for uint256;

  // The token being sold
  BosiToken public token; // = new BosiToken();
  ...
  function BosiCrowdsale(BosiToken _token, address _multisigVault) public {
    token = _token;
  ...
  function getTokenAmount(uint256 weiAmount) internal view returns(uint256) {
    return weiAmount.mul(tokensPerEther);   // note: both eth and BosiToken has 18 DECIMALS, hence omitting .div(10**(uint256(18) - TOKEN_DECIMALS))
  }
  ...
  // Allows owner to add accounts to the whitelist.
  // Only those whitelist accounts will be allowed to contribute more than CONTRIBUTION_MAX_NO_WHITELIST.
