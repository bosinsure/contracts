...
var BosiTokenMock = artifacts.require('mocks/BosiTokenMock.sol');
...
contract('BosiToken', function ([owner, beneficiary1, beneficiary2, beneficiary3]) {
  ...
  it('should return the correct totalSupply after construction', async function () {
    let token = await BosiTokenMock.new(owner, 100);
  ...
  it('should allow owner to set vesting grant', async function () {
    let token = await BosiTokenMock.new(owner, 1000);
    await token.setVestingGrant( beneficiary1, 100, start, cliff, vesting, false );
    assert.equal(await token.getVestingGrantAmount(beneficiary1), 100, "grant for the beneficiary1 should be set to 100");
    await token.transfer(beneficiary1, 100);
  });
  ...
  it('should monthly-graded release tokens during vesting period', async function () {
    ...
    await token.setVestingGrant( beneficiary1, 100, start, cliff, vesting, false );
    await token.transfer(beneficiary1, 100);
    let amount = await token.getVestingGrantAmount(beneficiary1);
    ...
    if (debug) console.log('Start='+ new Date(start * 1000), 'Cliff='+ new Date(cliff * 1000), 'Vesting='+ new Date(vesting * 1000));
    for (let i = 1; i <= checkpoints; i++) {
      ...
      if (debug) console.log('At time ' + i, new Date(t * 1000), 'Locked amount = ' + nonVestedAmount, ', vestedAmount = ' + expectedVested);
